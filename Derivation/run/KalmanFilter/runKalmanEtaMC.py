...
fJDATA=ROOT.TFile("JDATA13TeV.root")
...
def jacobian(line,state):
    ...
    term1 = a+k*e1*e1-1.0+1.0/(1.0+st1*m1*c1)+(b01+b11*sin(p1)+b21*sin(2*p1) +c11*cos(p1)+c21*cos(2*p1))/c1
    term2 = a+k*e2*e2-1.0+1.0/(1.0+st2*m2*c2)-(b02+b12*sin(p2)+b22*sin(2*p2) +c12*cos(p2)+c22*cos(2*p2))/c2
    ...
    if m1_bin != m2_bin:
        ...
        V[N] = (0.5/h) * (-st1*c1*term2/((1+st1*c1*m1)*(1+st1*c1*m1)))
        N = state.IToN['M_'+str(m2_bin)]
        V[N] = (0.5/h) * (-st2*c2*term1/((1+st2*c2*m2)*(1+st2*c2*m2)))
    else:
        N = state.IToN['M_'+str(m1_bin)]
        V[N] = (0.5/h) * (-st1*c1*term2/((1+st1*c1*m1)*(1+st1*c1*m1))-st2*c2*term1/((1+st2*c2*m2)*(1+st2*c2*m2)))
...
calibrator = KalmanCalibrator(infos,'calibrationMC13TeVETA.root')
calibrator.setModel(jacobian)
calibrator.update(ymc,1,lambda x,y: 9.45251)
calibrator.update(jmc,0,lambda x,y: 3.0952)
