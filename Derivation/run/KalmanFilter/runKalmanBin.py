...
pmap1_1= Partitioner([1.0/150000.,1.0/1.0],[-2.6,-2.0,-1.7,-1.4,-0.9,0.0,0.9,1.4,1.7,2.0,2.6],[-pi,pi],"")
...
def jacobian(line,state):
    ...
    term1 = a1-st1*m1*c1+(b01+b11*sin(p1)+b21*sin(2*p1) +c11*cos(p1)+c21*cos(2*p1))/c1
    term2 = a2-st2*m2*c2-(b02+b12*sin(p2)+b22*sin(2*p2) +c12*cos(p2)+c22*cos(2*p2))/c2
    ...
    if a1_bin != a2_bin:
        N = state.IToN['A_'+str(a1_bin)]
        V[N] = (0.5/h) *term2
        N = state.IToN['A_'+str(a2_bin)]
        V[N] = (0.5/h) *term1
    else:
        N = state.IToN['A_'+str(a1_bin)]
...
calibrator = KalmanCalibrator(infos,'calibrationMC13TeVBINNED.root')
