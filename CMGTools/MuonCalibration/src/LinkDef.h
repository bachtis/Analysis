#include "../interface/Roo3DMapVar.h"
#include "../interface/Roo2DMapVar.h"
#include "../interface/RooGaussianSumPdf.h"
#include "../interface/RooGaussianSumMultiPdf.h"
#include "../interface/RooGaussianSumPdfDouble.h"
#include "../interface/RooGaussianSumPdfWithSigma.h"
#include "../interface/RooGaussianSumPdfWithSigma2.h"
#include "../interface/RooPosNegBiasEstimator.h"
#include "../interface/RooSplineND.h"
#include "../interface/SampleSmearing.h"
#include "../interface/UnfoldingMap.h"
#include "../interface/UnfoldingMapDeriv.h"
#include "../interface/DynamicBinnedSmearingPdf.h"


#ifdef __CINT__
#pragma link off all globals;
#pragma link off all classes;
#pragma link off all functions;
#pragma link C++ class RooGaussianSumPdf+;
#pragma link C++ class RooGaussianSumMultiPdf+;
#pragma link C++ class RooGaussianSumPdfDouble+;
#pragma link C++ class RooGaussianSumPdfWithSigma+;
#pragma link C++ class RooGaussianSumPdfWithSigma2+;
#pragma link C++ class RooGaussianSumMultiPdf+;
#pragma link C++ class Roo3DMapVar+;
#pragma link C++ class Roo2DMapVar+;
#pragma link C++ class RooPosNegBiasEstimator+;
#pragma link C++ class RooSplineND+;
#pragma link C++ class SampleSmearing+;
#pragma link C++ class UnfoldingMap+;
#pragma link C++ class UnfoldingMapDeriv+;
#pragma link C++ class DynamicBinnedSmearingPdf+;

#endif

