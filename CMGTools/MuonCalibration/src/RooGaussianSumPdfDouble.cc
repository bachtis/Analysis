#include "../interface/RooGaussianSumPdfDouble.h"
#include "TMath.h"
#include "RooMath.h"
#include "DataFormats/Math/interface/LorentzVector.h"

ClassImp(RooGaussianSumPdfDouble)

RooGaussianSumPdfDouble::	RooGaussianSumPdfDouble(const char *name, const char *title,
			      RooAbsReal& _mass,
			      RooAbsReal& _scale,
			      RooAbsReal& _error1,
			      RooAbsReal& _error2,
...
RooGaussianSumPdfDouble::RooGaussianSumPdfDouble(const RooGaussianSumPdfDouble& other, const char* name) :
...
Double_t RooGaussianSumPdfDouble::evaluate() const
{
  Double_t arg= 0.0;
  Double_t sum=0.0;
  Double_t sumw=0.0;
  ...
  for (unsigned int i=0;i<data.size();++i) {
    ...
    arg = (mass-scale*data[i].second)/(error2);
    sum=sum+data[i].first*exp(-0.5*arg*arg);

    sumw=sumw+2*data[i].first;
  }
  return sum/sumw;
}

Int_t RooGaussianSumPdfDouble::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const 
{
...
Double_t RooGaussianSumPdfDouble::analyticalIntegral(Int_t code, const char* rangeName) const 
{
  static const Double_t root2 = sqrt(2.) ;
  static const Double_t rootPiBy2 = sqrt(atan2(0.0,-1.0)/2.0);
  Double_t xscale = root2*error1;
  Double_t xscale2= root2*error2;
  Double_t ret = 0;
  Double_t sumw = 0;
  ...
  for (unsigned int i=0;i<data.size();++i) {
    ...
    ret = ret+data[i].first*rootPiBy2*error2*(RooMath::erf((mass.max(rangeName)-scale*data[i].second)/xscale2)-RooMath::erf((mass.min(rangeName)-scale*data[i].second)/xscale2));
    sumw=sumw+2*data[i].first;
  }
  return ret/sumw ;
